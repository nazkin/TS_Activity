...
const Hero: React.FC<IHeroProps> = ({drinkList, recommended}) => {
  ...
  const drinksUI = drinkList.map(drink => {
    return (
      <Carousel.Item
        onClick={() => viewDrinkHandler(drink.id)}
        key={drink.id}
        interval={3000}
      >
        <img className="d-block w-100" src={drink.img} alt="First slide" />
        ...
        <Row className="recommended-row">
          <Col className="recommended-desc" xs={6}>
            <div>
              ...
              <h2 onClick={() => viewDrinkHandler(recommended.id)}>
                {recommended.name}
              </h2>
            </div>
            <section>{ingredientsUI}</section>
          </Col>
          <Col xs={6}>
            <img
              src={recommended.img}
              alt="recommended cocktail"
